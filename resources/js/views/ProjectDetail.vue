...
const canManageProject = computed(() => {
  ...
  if (authStore.user.id === project.value.owner_id) {
    return true;
  }

  // If not the owner, check if they are in the users list with a management role.
  if (!project.value.users) {
    return false;
  }
  const userInProject = project.value.users.find(u => u.id === authStore.user.id);
  return userInProject && ['manager', 'administrator'].includes(userInProject.pivot?.role);
});
