...
- `DemoDataSeeder.php` - **Mới**: Seed dữ liệu mẫu (projects, tickets, users...) để kiểm thử
...
- `ConfirmationModal.vue` - **Mới**: Modal xác nhận hành động (VD: xóa)
...
**Vai trò**: Chứa tất cả các tài liệu liên quan đến dự án.
- `README.md`: Hướng dẫn cài đặt và chạy dự án.
- `PROJECT_STRUCTURE.md`: Mô tả cấu trúc thư mục và các file quan trọng.
- `BUSINESS_LOGIC.md`: Mô tả chi tiết các yêu cầu nghiệp vụ của hệ thống.
- `API_COMPARISON.md`: So sánh độ bao phủ của API so với các tính năng của Filament.
- `changes.md`: Nhật ký các thay đổi quan trọng trong dự án.
